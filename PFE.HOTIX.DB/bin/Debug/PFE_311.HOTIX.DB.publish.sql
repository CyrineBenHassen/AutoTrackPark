...
IF EXISTS (select top 1 1 from [dbo].[def_user_role])
    RAISERROR (N'Lignes détectées. Arrêt de la mise à jour du schéma en raison d''''un risque de perte de données.', 16, 127) WITH NOWAIT
...
GO
PRINT N'Suppression de Contrainte par défaut contrainte sans nom sur [dbo].[app_user]...';


GO
ALTER TABLE [dbo].[app_user] DROP CONSTRAINT [DF__app_user__role_i__59FA5E80];
...
GO
ALTER TABLE [dbo].[app_user] DROP CONSTRAINT [DF__app_user__user_u__5AEE82B9];


GO
PRINT N'Suppression de Clé étrangère [dbo].[FK_appDemandeFormation_To_defUser]...';


GO
ALTER TABLE [dbo].[app_demande_formation] DROP CONSTRAINT [FK_appDemandeFormation_To_defUser];


GO
PRINT N'Suppression de Clé étrangère [dbo].[FK_appDemandeRecrutement_To_defUser]...';


GO
ALTER TABLE [dbo].[app_demande_recrutement] DROP CONSTRAINT [FK_appDemandeRecrutement_To_defUser];


GO
PRINT N'Suppression de Clé étrangère [dbo].[FK_AppUser_To_DefUserRole]...';


GO
ALTER TABLE [dbo].[app_user] DROP CONSTRAINT [FK_AppUser_To_DefUserRole];


GO
PRINT N'Début de la régénération de la table [dbo].[app_user]...';
...
CREATE TABLE [dbo].[tmp_ms_xx_app_user] (
...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[app_user])
    BEGIN
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_app_user] ON;
        INSERT INTO [dbo].[tmp_ms_xx_app_user] ([user_id], [role_id], [user_login], [user_password], [user_email], [user_phone], [user_is_active], [user_birth_date], [user_creation_date])
        SELECT   [user_id],
                 [role_id],
                 [user_login],
                 [user_password],
        ...
        SET IDENTITY_INSERT [dbo].[tmp_ms_xx_app_user] OFF;
    END

DROP TABLE [dbo].[app_user];

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_app_user]', N'app_user';
...
GO
PRINT N'Modification de Table [dbo].[def_user_role]...';


GO
ALTER TABLE [dbo].[def_user_role] ALTER COLUMN [role_code] VARCHAR (50) NULL;

ALTER TABLE [dbo].[def_user_role] ALTER COLUMN [role_name] VARCHAR (100) NULL;
...
GO
PRINT N'Actualisation de Procédure [dbo].[sp_user_role_get_all]...';


GO
EXECUTE sp_refreshsqlmodule N'[dbo].[sp_user_role_get_all]';
...
ALTER TABLE [dbo].[maintenance] WITH CHECK CHECK CONSTRAINT [FK_maintenance_To_type_maintenance];
